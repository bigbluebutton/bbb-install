...
main() {
  ...
  while builtin getopts "hs:r:c:v:e:p:m:xgadwji" opt "${@}"; do
...
check_version() {
  if ! echo "$1" | grep -Eq "focal-25"; then err "This script can only install BigBlueButton 2.5 (or later) and is meant to be run on Ubuntu 20.04 (focal) server."; fi
  DISTRO=${1%%-*}
